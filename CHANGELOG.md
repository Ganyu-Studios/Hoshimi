...
- e51c681: Filters are now active by presence, and any filter can be set — registered or not.
...
- e51c681: Fixes from an audit of the whole client:
  ...
  - **Queue**: `save()` measured `tracks.length` to trim the history, dropping entries well within `maxHistory`. `move(track, 0)` sent the track to the end instead of the front. `toJSON()` only threw on invalid tracks when _every_ track was invalid, and mutated `this.history` from inside a serializer. `splice()` duplicated the tracks it inserted into an empty queue, and `move()` cost three storage writes and three `queueUpdate` events.
